...
function ToggleClothing(whic, extra)
	...
	if which ~= 'Mask' then
		if not Gender then
			Notify("This ped model does not allow for this option")
			return false
		end
	end
	...
	if not Toggle.Table.Standalone then
		...
		Notify("There don't seem to be any variants for this")
		return
	else
		if not LastEquipped[which] then
	...
	Notify("You are already wearing that")
	return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		Notify("You don't appear to have anything to remove")
		return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then
...
function Notify(message, color)
	exports.ox_lib:notify({ description = message, type = color })
end
