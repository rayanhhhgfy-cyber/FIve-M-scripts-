...
RegisterNetEvent('qb-radialmenu:client:TakeStretcher', function()
    ...
    if vehicle ~= 0 then
        ...
        if obj ~= 0 then
            ...
            exports.ox_lib:notify({ type = 'error', description = "Couldn't create the requested object" })
        end
    else
        exports.ox_lib:notify({ type = 'error', description = "You're not near an Ambulance" })
    end
end)
...
RegisterNetEvent('qb-radialmenu:client:RemoveStretcher', function()
    ...
    if stretcherObject then
        ...
        if #(coords - bCoords) < 3.0 then
            ...
            exports.ox_lib:notify({ type = 'error', description = "You're too far away" })
        end
    end
end)
...
RegisterNetEvent('qb-radialmenu:client:Result', function(isBusy, type)
    ...
    if type == 'lay' then
        if not isBusy then
            ...
            exports.ox_lib:notify({ type = 'error', description = 'This stretcher is already in use' })
            isLayingOnBed = false
        end
    else
        if not isBusy then
