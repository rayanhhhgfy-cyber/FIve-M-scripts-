...
RegisterNetEvent('grapple:server:used', function()
    local src = source
    if not checkRateLimit(src, 'grappleUse', 10) then return end
    local player = QBox.Functions.GetPlayer(src)
    if not player then return end
    exports['discord-logs']:LogCustom(src, 'Grapple Used', 'Player used grappling hook')
end)
