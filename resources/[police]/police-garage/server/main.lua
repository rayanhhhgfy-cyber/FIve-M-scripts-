...
RegisterNetEvent('police:garage:server:vehicleSpawned', function(model, plate)
    local src = source
    ...
    exports['discord-logs']:LogCustom(src, 'Police Vehicle Spawn', model .. ' plate: ' .. plate)
end)

RegisterNetEvent('police:garage:server:vehicleStored', function(plate)
    local src = source
...
local function impoundAllVehicles()
    for src, vData in pairs(activeVehicles) do
        local player = QBox.Functions.GetPlayer(src)
        if player then
            TriggerClientEvent('police:garage:client:impoundVehicle', src)
            MySQL.insert('INSERT INTO police_vehicle_logs (citizenid, vehicle, plate, action, timestamp) VALUES (?, ?, ?, ?, ?)',
                { player.PlayerData.citizenid, vData.model, vData.plate, 'impounded_onduty_end', os.time() })
        end
    end
    activeVehicles = {}
end
...
QBox.Functions.CreateCallback('police:garage:server:getAvailableVehicles', function(source, cb)
    ...
    while true do
        ...
        for src, vData in pairs(activeVehicles) do
            ...
            if player then
                ...
                if job.type ~= 'leo' or not job.onduty then
                    if Config.PoliceGarage.SpawnSettings.impoundOnDutyEnd then
                        TriggerClientEvent('police:garage:client:impoundVehicle', src)
                        MySQL.insert('INSERT INTO police_vehicle_logs (citizenid, vehicle, plate, action, timestamp) VALUES (?, ?, ?, ?, ?)',
                            { player.PlayerData.citizenid, vData.model, vData.plate, 'impounded_auto', os.time() })
                        activeVehicles[src] = nil
                    end
                end
            end
